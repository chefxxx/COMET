...
struct Factorization {
    ...
    {
        long t = 1;
        for (auto& pa : m_prime_factors) t *= discreture::pow(pa.p, pa.a);
        return t;
    }
    ...
    long& operator[](long p)
    {
        auto it = std::partition_point(
            m_prime_factors.begin(), m_prime_factors.end(),
    ...
    void euler_sieve(long n)
    {
        ...
        for (long p = 3; p * p <= n; p += 2) {
            if (primecharfunc[p] == true) {
                for (long i = p * 2; i <= n; i += p) primecharfunc[i] = false;
            }
        }
...
inline std::vector<bool> operator&(const std::vector<bool>& A, const std::vector<bool>& B)
{
    ...
    for (size_t i = 0; i < size; i++) G.push_back(A[i] && B[i]);
