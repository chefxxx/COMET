...
class IndexedViewContainer
{
    ...
    auto operator[](size_type i) const { return indexed_view(objects_, indices_[i]); }

    class iterator : public boost::iterator_facade<
                         iterator, value_type, boost::random_access_traversal_tag, value_type>
    {
        public:
        using indices_iterator = typename ContainerOfIndexContainers::const_iterator;
        ...
        bool equal(const iterator& other) const { return indices_ == other.indices_; }

        value_type dereference() const { return indexed_view(objects_, *indices_); }
...
template <class Container, class ContainerOfIndexContainers>
auto indexed_view_container(const Container& A, const ContainerOfIndexContainers& I)
{
