...
class SetPartitions
{
    ...
    static bool next_set_partition(
        set_partition& data, const number_partition& part, difference_type n
    )
    {
    ...
    class iterator : public boost::iterator_facade<
                         iterator, const set_partition&, boost::forward_traversal_tag>
    {
        public:
        iterator() : ID_(0), data_(), n_(0) {}

        explicit iterator(IntType n, IntType numparts) : ID_(0), data_(n), n_(n), num_partition()
        {
            Partitions<IntType>::first_with_given_number_of_parts(num_partition, n, numparts);
    ...
    static difference_type NextAcceptablePlaceToAdd(
        const set_partition& data, const number_partition& part, difference_type oldpos = -1
    )
    {
    ...
    static bool shouldBreak(
        const set_partition& data, const number_partition& part, difference_type currpos,
