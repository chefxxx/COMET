...
BENCHMARK_DEFINE_F(CombinationsComparisonFixture, MyLib_FullPairs)(benchmark::State& state)
{
    for (auto _ : state) {
        // Your library performing Cartesian Product (N*N)
...
BENCHMARK_DEFINE_F(CombinationsComparisonFixture, Discreture_IntervalPairs)(benchmark::State& state)
{
    for (auto _ : state) {
        ...
        auto range = discreture::integer_interval(n); // [0, n)
...
BENCHMARK_DEFINE_F(CombinationsComparisonFixture, MyLib_FullTriples)(benchmark::State& state)
{
    for (auto _ : state) {
...
BENCHMARK_DEFINE_F(CombinationsComparisonFixture, Discreture_IntervalTriples)(benchmark::State& state)
{
    for (auto _ : state) {
        auto n = this->data.size();
...
BENCHMARK_REGISTER_F(CombinationsComparisonFixture, MyLib_FullPairs)
    ->RangeMultiplier(2)->Range(1 << 4, 1 << 11)->Complexity();
BENCHMARK_REGISTER_F(CombinationsComparisonFixture, Discreture_IntervalPairs)
    ->RangeMultiplier(2)->Range(1 << 4, 1 << 11)->Complexity();

BENCHMARK_REGISTER_F(CombinationsComparisonFixture, MyLib_FullTriples)
